...
function matchItem(kind){
  const type = qs('type');
  const img = qs('img');
  return getCatalog(kind).find(v => v.name===type) || getCatalog(kind).find(v => v.img===img) || getCatalog(kind)[0];
}
function mountOrderPage(kind, mode){
  const item = matchItem(kind);
  document.getElementById('orderTitle').textContent = `${kind==='corrugated' ? '골판지박스' : '칼라박스'} ${mode==='sample' ? '샘플 요청' : '견적문의'}`;
  document.getElementById('orderBadge').textContent = `${item.name} ${mode==='sample' ? '샘플 요청' : '견적문의'}`;
  ...
  document.querySelectorAll('.order-thumb').forEach((btn, idx)=>{
    if(btn.dataset.boxName===item.name) btn.classList.add('active');
...
function calculateEstimate(record){
  const qty = Number((record['수량']||'').toString().replace(/[^\d.]/g,'')) || 0;
  const w = Number((record['가로']||'').toString().replace(/[^\d.]/g,'')) || 0;
  const d = Number((record['세로']||'').toString().replace(/[^\d.]/g,'')) || 0;
  const h = Number((record['높이']||'').toString().replace(/[^\d.]/g,'')) || 0;
  const volume = (w*d*h)/1000000;
  const base = record.kind==='corrugated' ? 280 : 420;
  const typeAdj = record.kind==='corrugated'
    ? ({'A골':120,'B골':90,'E골':70,'BA골':220,'BB골':180}[record['골두께']] || 100)
    : ({'SC마닐라지':90,'아이보리지':130,'MGB지':150,'로얄아이보리지':170}[record['종이재질']] || 120);
  const printAdj = record.kind==='corrugated'
    ? ({'없음':0,'1도':40,'2도':80,'4도':160}[record['인쇄']] || 0)
    : ({'단면 4도':140,'양면 4도':220,'별색 인쇄':180}[record['인쇄선택']] || 0);
  const finishAdj = record.kind==='colorbox'
    ? ({'코팅안함':0,'오버코팅':40,'유광라미':90,'무광라미':110}[record['코팅선택']] || 0) + ({'없음':0,'금박':160,'은박':150,'형압':120,'부분UV':90}[record['옵션가공']] || 0)
    : 0;
  const unit = Math.round((base + volume*180 + typeAdj + printAdj + finishAdj) / Math.max(1, qty>=5000?1.35:qty>=1000?1.15:1));
  const total = unit * Math.max(qty,1);
  return {unit,total};
}
function saveEstimateRecord(form, mode){
  if(mode!=='quote') return;
  const fd = new FormData(form);
  const rec = {};
  fd.forEach((v,k)=> rec[k]=v);
  rec.kind = form.dataset.kind || '';
  rec.mode = mode;
  rec.createdAt = new Date().toISOString();
  const calc = calculateEstimate(rec);
  rec.estimatedUnitPrice = calc.unit;
  rec.estimatedTotalPrice = calc.total;
  const key = 'sdcompany_quotes_v1';
  const rows = JSON.parse(localStorage.getItem(key) || '[]');
  rows.unshift(rec);
  localStorage.setItem(key, JSON.stringify(rows.slice(0,300)));
}
function attachMailForm(formId, successText, mode='quote'){
