...
function mountLayout(active=''){
  const items=[['corrugated.html','골판지박스'],['colorbox.html','칼라박스'],['support.html','고객지원'],['quote.html','견적문의']];
  ...
  const appendFooter = ()=>{ if(!document.querySelector('.footer')) document.body.insertAdjacentHTML('beforeend', footerHtml); };
  if(document.readyState === 'loading'){ document.addEventListener('DOMContentLoaded', appendFooter, {once:true}); } else { appendFooter(); }
}
